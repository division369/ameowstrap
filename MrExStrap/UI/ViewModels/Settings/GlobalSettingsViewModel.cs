...
using GBS = ExploitStrap.Utility.GlobalBasicSettings;
...
namespace ExploitStrap.UI.ViewModels.Settings
{
    ...
    public class GlobalSettingsViewModel : NotifyPropertyChangedViewModel
    {
        ...
        private void Reset()
        {
            ...
            {
                Frontend.ShowMessageBox(
                    "Nothing to reset — ExploitStrap hasn't changed your Roblox settings, so there's no backup to restore.",
                    MessageBoxImage.Information);
                return;
            }

            var confirm = Frontend.ShowMessageBox(
                "This restores the Roblox settings exactly as they were before ExploitStrap first changed them. Continue?",
                MessageBoxImage.Warning, MessageBoxButton.YesNo);
